...
* Ergänzender Hinweis im Dialog bei Auswahl Merkmale "INSPIRE konform" bzw. "INSPIRE nicht konform" (#8565)
* Ersetzen der until-destroy Bibliothek (#8541)
* Import auch unterhalb von Metadatensätzen ermöglichen (#8475)
* IGE: BWaStr.-Strecke zusätzlich als BBOX hinzufügen (#8473)
...
* Nur Ordner mit veröffentlichten MD ans Portal liefern (#8418)
* IGE: Whitelist für URLs ohne Codeänderungen konifigurierbar machen (#8366)
* Beschreibungsfeld bei Sachattributen und Geometrien bitte dynamisch skalierbar machen (#8310)
* Editor: Validierung Datenformat in Katalogeinstellungen abschalten können (#8115)
* Umsetzungsticket, Raumbezugssystem-Codeliste durch Anbindung externe Registry erweitern (#7705)
* Umsetzungsticket: Verbesserung der Eingabe und Darstellung von Zeitbezügen (#7527)
* UVP-Editor - Aktivitätsbericht um Archivierung erweitern (#7525)
* ISO-XML von Dienst-Metadaten bereinigen: Abschnitt srv:coupledResource entfernen (#7233)
* "Übernahme freie Einträge" Funktionalität aus IGE Classic (#7063)
* Anzeige von Projektnummer und Projekttitel im Portal (#6713)
* Zoneless Angular (#6408)
* "Amtsintern" als default Einstellung für neue Datensätze (#6352)
* IGE: neue Dienstart "OGC API-Feature" (#6189)
* Anzeige WKTs im Editor verbessern (#5731)

### Bugfixes

* INSPIRE nicht konform Verhalten mit nicht evaluierten Konformität (#8589)
* Fehler beim Wechseln eines Datensatzes (#8573)
* Bug bei der Indizierung von LFS Links  (#8557)
* HTML-Ausgabe der OGC-API verbessern (#8553)
* Direkte Elasticsearch-Indizierung verliert Aktivierungsstatus (#8547)
* Fehler beim Entfernen einer Operation (#8539)
* Favoriten in kataloginternen Codelisten werden überschrieben/zurückgesetzt (#8531)
* Fehlende Informationen zum Raster-/Gridformat (#8509)
* Unerwünschte Anzeige Identifikator im Portal auf TEST (#8488)
* Editor: Anzeige langer Namen von hochgeladenen Dateien anpassen (#8480)
* Vorschau stimmt nicht mit dem Datensatz  Inhalt (#8474)
...
* IGE: Umbenennung eines Ordners wird beim Erstellen einer Adresse nicht berücksichtigt (#8385)
* Formular kann nicht seitlich gescrollt werden (#8348)
* UVP Nummer optionen werden weiter angezeigt nach verhalten deaktivieren (#8304)
* Automatisierte Rollenzuweisung im Keycloak funktioniert nicht (#8300)
* Dashboard - "Zuletzt Veröffentlicht im Katalog" fehlerhaft, schließt Daten mit Bearbeitungsversion aus (#8116)
* Kontexthilfe bei "Dateien" fehlt in manchen Objektklassen (#8031)
* ISO Import übernimmt gco:Date nicht im erwarteten ISO-Zeitformat (#7776)
...
* Anzeige von Datensätzen mit Schreibrechten (#8163)
* ISO-Import mit Codelist-IDs und Values (#8119)
* Migration, Fehlerbehebung und weitere Themen (#8111)
* Änderung Fehlertext für LUBW-Profil (#8110)
* Andere "Hilfe" für Redakteure verlinken (#8109)
* Umsetzung Zusammenführung Datenrepository / Datenfinder (#8073)
* "Funktion/Button “Meine Objektarten(en)” im Editor für die Rolle Redakteur (#8039)
* Capabilities-Assistent in Diensten, auf denen jemand nur Leserechte hat, ausblenden (#8038)
* Kryptische Fehlermeldung sollte leicht verständlicher Text werden (#8027)
* Einschränkung des Funktionsumfangs für Rolle "Redakteur" (#7966)
* OGC API für EIA Format ertüchtigen (#7832)
* Aufräum-Job für Zabbix für Verfahren (#7824)
* E-Mail über abgelaufene Metadaten anpassen (#7785)
* Passwortgeschützte Dienste über Capabilites-Assistent anlegen (#7750)
* Inhalte der Migrations-Tabellen (Objektart, Merkmal, Geometrie) (#7602)
* Filterung/Kennzeichnung von WWA-AB-Daten im LfU-Bayern Portal (#7548)
* Export von Metadaten aus dem IGE direkt in eine CSW Schnittstelle (#7531)
* Refactoring der IGE-NG Dashboard Statistik (#7440)
* SKDV-OK, Definition Felder für Redakteure (#7226)
* Serverseitige Validierung beim Einliefern über die OGC-API Records Schnittstelle (#7211)
* "Merkmale" - Dialoge und Hilfetexte überarbeiten (#7151)
* Editor: Datenformat - Pflichtangabe bei INSPIRE-relevanten Diensten (#7048)
* Erstellung Formular für Objektarten im Editor (#7018)
* Neugestaltung: Integration vom File Store (LFS) im IGE (#6971)
* Umsetzung Profil IGE-NG für die BAW (#6790)
* Verbesserung der Robustheit der Synchronisation mit UVP Monitoring (#6710)
* Person, die Import durchführt als "Verantwortlich" setzen (#6265)
* Stacktraces aus Fehlerresponses entfernen (#5773)

### Bugfixes

* Falsche Berechtigungsprüfung beim Import (#8318)
* CSW-T Insert verursacht NullPointerException (#8308)
* Kopieren mit Teilbaum führt zur Fehlfunktion bei hochgeladenen Dateien (#8293)
* Falscher Style im Hierarchiebaum für einen Baumknoten (#8283)
* JSON-Schema Fehler bei fehlender Service-URL Erklärung (#8267)
* Amtsinterne Adressen nicht als untergeordnete Adressen anzeigen (#8196)
* DOI Codelistenwerte nicht auswählbar (#8175)
* Aktualisierung der Bibliotheken (#8151)
* 'Zugang geschützt' Label fehlt in der Detailansicht (#8142)
* Button nicht richtig zentriert (#8104)
* Eintrag "WMS-URL"  bzw. "Dienste-URL" im Anwendungsprofil ist verloren gegangen (#8082)
* Report - Abgelaufene Metadaten - Veröffentlicht mit Bearbeitungsversion wird nicht berücksichtigt (#8081)
* Wiedervorlage E-Mail muss aus Profil genommen werden (#8072)
* IGE: Geodatendienste: Systemumgebung und Historie erscheinen nicht unter "Weitere Informationen" wie bei der Anwendung (#8066)
* ISO Import enthält unnötige Leerzeichen und Zeilenumbrüche - URI von Dateien können nicht gematch werden (#8063)
* Keyword wird nicht hinzugefügt "hmbtg" bei  "Veröffentlichung gemäß HmbTG" (#8051)
...
* Langsames Drag'n'Drop Verhalten bei Vorschaugrafiken (#8025)
...
* Aktualität der Metadaten bei Export/Import (#7872)
* JSON-Validierungsfehler Raster-/Gridformat (#7800)
* IGE-NG - Vergleichsansicht: OpenData / HVD wird nicht hervorgehoben (#7699)
* Thesauri laden sehr langsam (#7247)
* Anzeige von leeren Ordnern im Portal trotz Löschung der enthaltenen veröffentlichten Daten  (#6230)
* serviceType bei INSPIRE-relevanten Diensten (#5806)
* Mapclient: Für WMS-Dienste von MV werden die Attributinformationen (GetFeatureInfo-Attribute) nicht angezeigt (#5439)
* Verbesserung der Fehlermeldung im Upload Dialog (#5118)
