...
`$KEYCLOAK_HOST`, `$REALM`, `$USERNAME`, `$PASSWORD`, `$CLIENTID`
...
| POST | Distributions
| Upload multiple distributions (only file upload; no document updates)

Endpoint: ```/api/ogc/collections/{collectionId}/items/{recordId}/distributions```

| DELETE | Distributions
| Delete a distribution (only file deletion; no document updates)

Endpoint: ```/api/ogc/collections/{collectionId}/items/{recordId}/distributions?uri={fileUri}```
