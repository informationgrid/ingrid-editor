...
==== OGC API Customization
This section provides an overview of custom OGC endpoints.
...
| Get information about resource/resources

Endpoint: ```/api/ogc/collections/{collectionId}/items/{recordId}/resources```

| GET | Resource
| Download a resource

Endpoint: ```/api/ogc/collections/{collectionId}/items/{recordId}/resources/download?uri={fileUri}```

| POST | Resources
| Upload multiple resources (only file upload; no document updates)

Endpoint: ```/api/ogc/collections/{collectionId}/items/{recordId}```

| DELETE | Resource
| Delete a resource (only file deletion; no document updates)

Endpoint: ```/api/ogc/collections/{collectionId}/items/{recordId}?uri={fileUri}```
