...
@Profile("<name>")
@Configuration
@ComponentScan(basePackages = ["de.ingrid.igeserver.profiles.<name>"])
class <Name>Config
----
...
@NgModule({
  declarations: [<Profile>Component],
})
...
** override at least the fields: `id`, `identifier`, `title` and `parentProfile`
