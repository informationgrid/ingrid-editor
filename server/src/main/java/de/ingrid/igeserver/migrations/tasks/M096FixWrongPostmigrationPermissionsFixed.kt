...
// fixed version of M089FixWrongPostmigrationPermissions, where NULL-values were not handled
@Service
class M096FixWrongPostmigrationPermissionsFixed : MigrationBase("0.96") {
    val log = logger()
        ...
        ClosableTransaction(transactionManager).use {
            ...
            entityManager
                ...
                WHERE acl.object_id_identity = dw.id\:\:varchar(255)
                  AND acl.parent_object IS DISTINCT FROM parent_acl.id;
                    """.trimIndent(),
                )
                .executeUpdate()
        }
