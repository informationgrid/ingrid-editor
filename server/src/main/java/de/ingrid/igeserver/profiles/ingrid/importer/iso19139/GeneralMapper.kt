...
open class GeneralMapper(val isoData: IsoImportData, val uploadConfig: UploadConfig) {
    ...
    fun getGraphicOverviews(): List<PreviewGraphic> = metadata.identificationInfo[0].identificationInfo?.graphicOverview
        ?.map {
            val isInternalStorage: Boolean = it.mdBrowseGraphic?.fileName?.value?.contains(uploadConfig.uploadExternalUrl ?: "/documents/") ?: false
