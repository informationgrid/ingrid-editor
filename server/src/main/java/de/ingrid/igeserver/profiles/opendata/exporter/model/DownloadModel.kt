...
data class DownloadModel(val title: String?,
    ...
    fun getLicenseData(catalogId: String): Any? {
        if(license != null) {
            ...
            val entryID = license.key ?: OpenDataModel.codeListService?.getCodeListEntryId("6500", license.value, "de")
            if(entryID != null) {
                val codeListEntry = OpenDataModel.codeListService?.getCodeListEntry("6500", entryID)
            ...
            OpenDataModel.codelistHandler?.getCatalogCodelistValue(catalogId, "20004", license.key!!)?.let { 
