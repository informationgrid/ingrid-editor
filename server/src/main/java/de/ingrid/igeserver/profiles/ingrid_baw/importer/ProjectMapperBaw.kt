...
class ProjectMapperBaw(isoData: IsoImportData) : GeneralMapper(isoData) {
    ...
    fun getOrderNumber(): String? = identificationInfo?.citation?.citation?.identifier?.firstOrNull()?.mdIdentifier?.code?.value
}
