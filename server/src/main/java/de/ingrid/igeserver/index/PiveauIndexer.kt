...
class PiveauIndexer(override val name: String, private val client: PiveauClient) : IIndexManager {
    ...
    override fun createIndex(name: String, type: String, esMapping: String, esSettings: String): Boolean = runBlocking {
        if (!client.catalogueExists(catalogId)) {
            client.createCatalogue(catalogId, name)
        }
        true
    }
