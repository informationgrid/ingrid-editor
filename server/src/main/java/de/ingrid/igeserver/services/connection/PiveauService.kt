...
@Service
class PiveauService(val settingsService: SettingsService) : IConnection {

    val log = logger()

    private var clientMap: Map<String, PiveauClient> = emptyMap()
    ...
    fun setupConnections() {
        try {
            val piveauServiceConfig = settingsService.getPiveauConfig()
            clientMap =
                piveauServiceConfig.associate { it.id!! to createPiveauClient(it) }
        } catch (e: Exception) {
            log.error("Could not connect to Piveau-interface", e)
            clientMap = emptyMap()
        }
    }

    private fun createPiveauClient(config: PiveauConfig): PiveauClient = PiveauClient(
    ...
    fun getClient(index: String): PiveauClient = clientMap[index] ?: throw ServerException.withReason("No Piveau-Client found. Please check the connection.")
    ...
    override fun isConnected(id: String): Boolean = runBlocking {
        try {
            ...
            log.warn("No connection to Piveau Service '$id': ${e.message}")
            false
        }
    }
