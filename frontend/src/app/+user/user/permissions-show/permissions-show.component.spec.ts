...
describe('PermissionsShowComponent', () => {
  ...
  it('should show permissions grouped by their types', () => {
    ...
    expect(spectator.queryAll('.permissions .action .permission').length).toBe(1);
    expect(spectator.queryAll('.permissions .documents .permission').length).toBe(1);
    expect(spectator.queryAll('.permissions .addresses .permission').length).toBe(1);
  });
});
