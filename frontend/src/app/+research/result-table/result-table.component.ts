...
export class ResultTableComponent implements OnInit, AfterViewInit {
  ...
  downloadTable() {
    const rows: string[][] = [];
    rows.push(["ID", "Veröffentlichungsstatus", "Veröffentlichungsrecht", "Typ", "Titel", "Aktualität"]);
  ...
  private buildRowByDoc(doc: IgeDocument) {
    return [doc._uuid, doc._state, doc._tags, doc._type, doc.title, doc._contentModified];
  }
}
