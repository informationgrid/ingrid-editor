 ...
 * OpenDataBehaviour
 */
export class TitleValidatorBehaviour
  ...
  implements Behaviour
{
  ...
  register(form: FormGroup, eventManager: EventManager) {
    form.get("title").validator = function (fc: FormControl) {
      return fc.value && fc.value.length >= 3
        ? null
        : {
            validateTop: {
              valid: false,
              error: "Der Titel muss aus mindestens 3 Zeichen bestehen",
            },
          };
    };
    ...
    form.get("taskId").validator = function (fc: FormControl) {
      ...
      return fc.value && fc.value.length > 0
        ? null
        : {
            validateTop: {
              valid: false,
