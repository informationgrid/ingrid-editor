...
import {FormToolbarService, ToolbarItem} from '../../../../+form/form-shared/toolbar/form-toolbar.service';
...
export class TreeModeToolbarBehaviour extends Plugin {
  ...
  register() {
    ...
    this.query.multiSelectMode$.subscribe(multiSelectMode => this.handleMode(multiSelectMode));
  }
  ...
  private handleMultiSelectMode() {
    ...
    this.previousState = this.toolbarService.buttons
      .map(button => ({id: button.id, active: (<ToolbarItem>button).active}));

    this.toolbarService.buttons
      .filter(button => !this.activeToolbarItemsInMultiSelect.find(id => button.id === id))
      .forEach(button => this.toolbarService.setButtonState(button.id, false));
